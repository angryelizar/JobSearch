...
values
    ((select id from RESUMES where id = 2), 8, 'Интеллика', 'SMM-менеджер', 'Ведение проектов'),
    ((select id from RESUMES where id = 3), 2, 'Attractor School', 'Junior Java-developer', 'Ассистент преподавателя');

insert into EDUCATION_INFO (RESUME_ID, INSTITUTION, PROGRAM, START_DATE, END_DATE, DEGREE)
values
    ((select id from RESUMES where id = 2), 'Bold Brands Education', 'SMM за полгода',  '2015-03-01', '2015-06-01', 'SMM-менеджер'),
    ((select id from RESUMES where id = 3), 'Attractor School', 'Java за полгода',  '2016-03-01', '2016-06-01', 'Junior Java-developer');
...
values
    ((select id from RESUMES where id = 2), (select id from VACANCIES where id = 1), true),
    ((select id from RESUMES where id = 3), (select id from VACANCIES where id = 2), true);

insert into MESSAGES (RESPONDED_APPLICANTS, CONTENT, "date_time")
values
    ((select id from RESPONDED_APPLICANTS where id = 1), 'Салам молекулам!', '2024-03-10 16:20:00');